...
function useFetch<T>(
    url: string,
    resultHandler: (this: Response) => Promise<T>,
    defaultValue?: T
): [value: T | undefined, reload: () => void] {
    ...
    const tryFetch = useCallback(() => {
        ...
        const handler = async () => {
            try {
                ...
                if (result.ok) setValue(await resultHandler.bind(result)());
...
function useFetchString(
    url: string
): [value: string | undefined, reload: () => void];
function useFetchString(
    url: string,
    defaultValue: string
): [value: string, reload: () => void];
function useFetchString(url: string, defaultValue?: string) {
    return useFetch(url, Response.prototype.text, defaultValue);
}

function useFetchJson<T = unknown>(
    url: string
): [value: T | undefined, reload: () => void];
function useFetchJson<T = unknown>(
    url: string,
